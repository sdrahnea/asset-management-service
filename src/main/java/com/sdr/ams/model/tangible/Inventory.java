...
@Entity
@AttributeOverride(name = "name", column = @Column(name = "inventory_id", nullable = false, length = 100))
@Table(
	name = "inventory",
	uniqueConstraints = {
		@UniqueConstraint(name = "uk_inventory_inventory_id", columnNames = "inventory_id"),
		@UniqueConstraint(name = "uk_inventory_serial_number", columnNames = "serial_number")
	}
)
public class Inventory extends CoreEntity {
	...
	@Size(max = 100, message = "Inventory ID must be at most 100 characters")
	@Pattern(regexp = "^[A-Za-z0-9_-]+$", message = "Inventory ID must be alphanumeric")
